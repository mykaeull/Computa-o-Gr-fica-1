    ...
    bool has_shadow(Vector pi, Vector L, double length_Pf_Pi)
    {
        ...
        for (int i = 0; i < objects.size(); i++)
        {
            ...
            if (objects[i].type == "cone")
            {
                ...
                Vector N_base = Vector(objects[i].u.x, objects[i].u.y, objects[i].u.z);
                Vector P_pi_base = objects[i].base;
                Object plane_base("plane", P_pi_base, N_base, objects[i].specular, objects[i].k_d, objects[i].k_e, objects[i].k_a, "base");
                s1 = intersect_ray_base(pi, L, plane_base, objects[i]);
                if (s1 > 0 && s1 < length_Pf_Pi)
                {
                    shadow = true;
                }
...
int main()
{
    ...
    Object sphere1("sphere", raio, Vector(0, 0, -100.), 10., Vector(0.7, 0.2, 0.2), Vector(0.7, 0.2, 0.2), Vector(0.7, 0.2, 0.2));

    objects.push_back(sphere1);

    Object plane1("plane", Vector(0, -raio, 0), Vector(0., 1., 0.), 1., Vector(0.2, 0.7, 0.2), Vector(0, 0, 0), Vector(0.2, 0.7, 0.2), "floor");
    Object plane2("plane", Vector(0, 0, -200.), Vector(0., 0., 1.), 1., Vector(0.3, 0.3, 0.7), Vector(0, 0, 0), Vector(0.3, 0.3, 0.7), "background");

    objects.push_back(plane1);
    objects.push_back(plane2);

    Object cylinder1("cylinder", raio / 3., Vector(0, 0, -100.), raio * 3, Vector(-1 / sqrt(3), 1 / sqrt(3), -1 / sqrt(3)), 10., Vector(0.2, 0.3, 0.8), Vector(0.2, 0.3, 0.8), Vector(0.2, 0.3, 0.8));
    objects.push_back(cylinder1);

    Object cone1("cone", raio * 1.5, Vector(-69.282, 69.282, -169.282), raio / 3, Vector(-1 / sqrt(3), 1 / sqrt(3), -1 / sqrt(3)), 10., Vector(0.8, 0.3, 0.2), Vector(0.8, 0.3, 0.2), Vector(0.8, 0.3, 0.2));
